...
CI (`.github/workflows/node.js.yml`) runs `npm ci`, `npm run build` (with Firebase/PostHog public env vars injected from GitHub secrets), then `npm run lint` on every push/PR to `main`. A separate workflow (`.github/workflows/build.yml`) runs a SonarQube scan on push to `main`. `.github/workflows/cleanup-history.yml` runs `scripts/cleanup-history.js` nightly via cron to purge Firestore chat history older than 30 days.
...
- `history/*` — sent-message log for the dashboard's History tab; pruned after 30 days by `scripts/cleanup-history.js`.
- `private/config` — secret `apiToken` for the remote-control HTTP API (see below). `private/twitch` — the user's Twitch OAuth access token.
